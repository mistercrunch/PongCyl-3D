...
*/
//Basic 3d vector class. I wish we could overload operators in Java...
class Vect3d
{
  float x,y,z;
  ...
  float DistanceXY()
  {
    float f = pow((float)x,2) + pow((float)y,2);
    return pow(f, 0.5);
  }
  ...
  float DistanceXY(Vect3d p)
  {
    float f = pow((float)x-p.x,2) + pow((float)y-p.y,2) ;
    return pow(f, 0.5);
  }
  ...
  void DriftTowards(Vect3d p, float PercentageDistance)
  {
    
    x += (p.x-x) * PercentageDistance;
    y += (p.y-y) * PercentageDistance;
    z += (p.z-z) * PercentageDistance;
  }
  void Add(Vect3d p)
  {
    x += p.x;
